            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <?php
                            $where = ($cid !== 'all' && $cid > 0) ? " AND category_id = '$cid'" : "";
                            $query = "SELECT * FROM products WHERE unix_timestamp(bid_end_datetime) >= " . strtotime(date("Y-m-d H:i")) . $where . " ORDER BY name ASC";
                            $cat = $conn->query($query);
                            if ($cat->num_rows <= 0) {
                                echo "<center><h4>No Available Products, Please check Later.</h4></center>";
                            }
                            while ($row = $cat->fetch_assoc()):
                            ?>
                                <div class="col-sm-4">
